...
export default function CompanyForm({ handleNext }) {
    ...
    const editRecruiterSchema = Yup.object().shape({
        ...
        selectRecruiter: Yup.object().nullable(),
        // recruiterName: Yup.string().required("Name is required"),
        recruiterName: Yup.object().nullable(),
    ...
    const formik = useFormik({
        ...
        onSubmit: (v) => {
            ...
            const data = {
                ...
                recruiterName: (user?.role === 'RECRUITER') ? user?.recruiterName : v?.recruiterName?.label,
                email: (user?.role === 'RECRUITER') ? user?.email : v?.recruiterName?.email,
                phone: v?.phone,
    ...
    async function updateRecruiter(data) {
        const res = await recruiterServices.adminUpdate(data)
        setSubmitting(false)
        if (res && res.success) {
                                    ...
                                    (
                                        <Stack>
                                            ...
                                            <Autocomplete
                                                ...
                                                onChange={(event, value) => { setFieldValue("recruiterName", value) }}
